...
@Validated
@Data
public class IESalaryService2015 {
	
	private Logger log  = LogManager.getLogger(this.getClass());
	
	BigDecimal MaxSal = bd("18300");
	
	private static String[][] uscA = { {"1.5", "12012"}, // up to 12012
										{"3.5", "17576"},
										{"7", "70044"},
										{"8", "99999999"}};
	
	private static String[][] uscS = { {"1.5", "12012"},
										{"3.5", "17576"},
										{"7", "70044"},
										{"8", "100000"},
										{"11", "99999999"}};
	
	private static String[][] uscR = { 	{"1.5", "12012"},   // Reduced usc for MedicalCard holders and those over 70 iff total earnings less than 60000
										{"3.5", "999999"}};
	private static final BigDecimal uscReducedMax = bd("60000");
	
	private static String[][] prsiA = { {"0", "8.5", "352"},
										{"4", "8.5", "356"},
										{"4", "10.75", "500"}};
	
	private static String[][] prsiS = {	{"4", "0", "500"}};
	 ...
	 * 
	 * @param s
	 * @return Tuple, Prsi, Employers, CostOfEmployment
	 */
	...
	public Tuple3<BigDecimal, BigDecimal, BigDecimal> calcPrsi(SalaryInputParams s){
		...
		while(lim<prsiRatesTable.length-1 && s.getGross().compareTo(prsiRatesTable[lim][2])>0)
			lim++;
		prsi = prsiRatesTable[lim][0].multiply(s.getGross());
		if(s.getPrsiClass()==Prsi.S)
	...
	public Salary[] addUsc(Salary[] sals){
		...
		for(int i=0;i<sals.length;i++){ 
			...
			if(s.isEligbleForReducedUsc() && s.getGross().compareTo(uscReducedMax)<=0)
			...
			Tuple3<BigDecimal, Integer, BigDecimal> calcResp = calcUscRecur(runningTotalGross, s.getGross(), uscRateTable, idx, runningTotalInBand);
